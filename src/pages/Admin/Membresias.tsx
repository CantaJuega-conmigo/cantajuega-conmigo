...
import {
  deleteSubscribeMembreship,
  getAllMebreships,
} from "@/functions/memership.query";
...
export default function Membresias() {
  const [membership, setMemberships] = useState<state[]>([
    {
      id: "",
      name: "",
      description: "",
  ...
  const PreConfirm = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    setSeeAlerts({
      text1: "DESEA GUARDAR LOS CAMBIOS?",
      text2: "Los cambio se guardaran  en la pagina web.",
      CancelText: "Cancelar",
      AcceptTText: "Guardar",
    });
  };
  ...
  const deleteMembershipsDB = () => {
    DeleteList.length &&
      DeleteList.forEach((value) => deleteSubscribeMembreship(value));
  };
  return (
    <div className="  min-h-screen flex flex-col gap-40">
      ...
      <section className=" w-[70%] m-auto  grid grid-flow-row  grid-cols-2  gap-8 ">
        {membership.map((i, key) => (
          <article key={key} className=" m-auto">
            <MembershipCard
              onClick={(event) => deleteInComponent(event)}
              membership={i}
              color={color[key]}
      ...
      <section className="w-full flex flex-col items-center gap-5">
        <button className="text-white bg-black text-3xl h-[3rem] w-[3rem] rounded-full">
          +
        </button>
        <article className="w-full flex justify-center gap-5">
          <button
            className="border-2 border-black p-3 w-[10rem]"
